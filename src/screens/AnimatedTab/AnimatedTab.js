...
import { View, Text, Alert, FlatList, Animated, Image, Dimensions, StyleSheet, StatusBar, findNodeHandle } from 'react-native'
...
const Slides = ({ item, index }) => <Image source={item.url} style={{ width, height, flex: 1, resizeMode: 'contain' }} />
...
const Tabs = ({ scrollX, data }) => {
  ...
  useEffect(() => {
    ...
    for (let mine of data) {
      mine.ref.current.measureLayout(containerRef.current, (x, y, width, height) => m.push({ x, y, width, height }))
