...
const Indicator = ({ scrollY }) => {
  const y = scrollY.interpolate({
    ...
    outputRange: [-INDICATOR_WIDTH - INDICATOR_MARGIN * 2, 0, INDICATOR_WIDTH + INDICATOR_MARGIN * 2],
  })
...
const RenderItem = ({ item, index }) => {
  return (
    <View style={{ width, height: ITEM_HEIGHT }}>
      <Image source={item.url} style={{ flexGrow: 1, height: null, width: null, resizeMode: 'contain' }} />
    </View>
  )
}
