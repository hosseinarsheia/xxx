    ...
    <View style={styles.paginationWrapper}>
      {PRODUCT_LIST.map((value, index) => {
        ...
        const animatedStyle = useAnimatedStyle(() => ({
          transform: [{ translateX: interpolate(scrollX.value, inputRange, [-INDICATOR_WIDTH, 0, INDICATOR_WIDTH]) }],
        }))
  ...
  return (
    <View style={styles.circleWrapper}>
      {PRODUCT_LIST.map((value, index) => {
        ...
        return <Animated.View key={`${index}`} style={[styles.circle, { backgroundColor: value.bg }, animatedStyle]} />
      })}
    </View>
  )
}
...
const HeaderTextComonent = ({ scrollX }) => {
  ...
  return (
    <View style={styles.headerTextStyle}>
      {PRODUCT_LIST.map((value, index) => {
        const animatedStyle = useAnimatedStyle(() => ({
          transform: [{ translateY: interpolate(scrollX.value, inputRange, [HEADER_HEIGHT, 0, -HEADER_HEIGHT]) }],
        }))
        return (
